...
public class BizOrderServiceImpl implements BizOrderService {
    ...
    @Transactional
    @Override
    public BizOrder createOrder(PayProduct payProduct, CustomerBindCardResult customerBindCardResult, PayRequestVo payRequestVo) {
        try {
            checkNotNull(payProduct);
            checkNotNull(customerBindCardResult);
            checkNotNull(customerBindCardResult.getCustomerAccount());
            checkNotNull(customerBindCardResult.getPaymentMethod());
            checkNotNull(payRequestVo);

//            Long payAmount = Long.valueOf(AmountUtil.convertDollar2Cent(payRequestVo.getTotalAmount()));
            BizOrder bizOrder = fillInfo(payProduct, customerBindCardResult.getCustomerAccount(), payRequestVo);

            bizOrderMapper.save(bizOrder);
            serviceTimeMapper.saveMore(bizOrder.getServiceTimes());

            PayProductTypeEnum pt = payProduct.getProductType();
            if (pt.equals(PayProductTypeEnum.OIL_CHANGE) || pt.equals(PayProductTypeEnum.PRE_INSPECTION) || pt.equals(PayProductTypeEnum.TRADE_IN)) {

                AppointmentDoc appointmentDoc = appointmentDocMapper.findByAppointmentId(bizOrder.getDocId());
                if (appointmentDoc == null || (!appointmentDoc.getAppointmentStatus().equals(AppointmentStatus.SAVED))) {
                    throw new HiveelBizException(HiveelPayServiceErrorCode.INVALID_APPOINTMENT_ERROR);
                }
                // docId is appointmentId
                int i = appointmentDocMapper.updateAndAssociateBizOrderNo(bizOrder.getDocId(), bizOrder.getBizOrderNo(), AppointmentStatus.SAVED);
                if (i == 1) {
                    _log.info("Associate->bizOrderNO:{},appointmentId:{} ", bizOrder.getBizOrderNo(), bizOrder.getDocId());
                } else {
                    _log.warn("BizOrder:{},was not associate with {}, productType:{}", bizOrder.getBizOrderNo(), bizOrder.getDocId(), pt.getName());
                }
            }

            if (pt.equals(PayProductTypeEnum.ADVANCING) ||
                    pt.equals(PayProductTypeEnum.SEARCH_RESULT) ||
                    pt.equals(PayProductTypeEnum.HIGHLIGHTING) ||
                    pt.equals(PayProductTypeEnum.CAR_OF_DAY)
            ) {
                bizCarMapper.save(bizOrder.getBizCar());
            }

            return bizOrder;
        } catch (Exception e) {
            _log.error("Save BizOrder error!", e);
            throw new HiveelPayException(HiveelPayServiceErrorCode.DATA_SAVE_BIZ_ORDER_ERROR, e);
        }
    }

    private BizOrder fillInfo(PayProduct payProduct, CustomerAccount customerAccount, PayRequestVo payRequestVo) {
        BizOrder bizOrder = new BizOrder();
        bizOrder.setCustomerId(customerAccount.getCustomerId());
        final String bizOrderNo = MySeq.getBizOrderId();

        bizOrder.setBizOrderNo(bizOrderNo);
        bizOrder.setProductId(payProduct.getProductId());
        bizOrder.setProductName(payProduct.getProductName());
        bizOrder.setAmount(payProduct.getAmount());
        bizOrder.setProductType(payProduct.getProductType().name());
        bizOrder.setServiceLength(payProduct.getServiceLength());
        bizOrder.setServiceLengthUnit(payProduct.getServiceLengthUnit());
        bizOrder.setOrderStatus(BizOrderStatus.SAVED);
        bizOrder.setPayAmount(Long.valueOf(AmountUtil.convertDollar2Cent(payRequestVo.getTotalAmount())));
        bizOrder.setFirstBillDate(DateUtil.strToDate(payRequestVo.getFirstBillDate(), FORMAT_MMddyyyyHHmm));
        bizOrder.setCommissionAmount(0L);

        List<ServiceTime> serviceTimes = Lists.newArrayList();
        for (String serviceTime : payRequestVo.getServiceTimes().split(";")) {
            if (Strings.isNullOrEmpty(serviceTime)) {
                continue;
            }
            ServiceTime st = new ServiceTime();
            st.setBizOrderNo(bizOrderNo);

            if (serviceTime.contains(",")) {

                String[] stArr = serviceTime.split(",");
                if (!Strings.isNullOrEmpty(stArr[0])) {
                    st.setServiceStartTime(DateUtil.strToDate(stArr[0], FORMAT_MMddyyyyHHmm));
                } else {
                    throw new HiveelCheckoutException(HiveelPayErrorCode.ILLEGAL_PARAMETERS, "Service start-time must not null and format like 'MMddyyyyHHmm'.");
                }

                if (!Strings.isNullOrEmpty(stArr[1])) {
                    st.setServiceEndTime(DateUtil.strToDate(stArr[1], FORMAT_MMddyyyyHHmm));
                } else {
                    throw new HiveelCheckoutException(HiveelPayErrorCode.ILLEGAL_PARAMETERS, "Service end-time must not null and format like 'MMddyyyyHHmm'.");
                }
            } else {
                st.setServiceStartTime(DateUtil.strToDate(serviceTime, FORMAT_MMddyyyyHHmm));

                if (payProduct.getServiceLength() == 0) {//无限，截止日期到2060年
                    st.setServiceEndTime(DateUtil.strToDate("010120600000", FORMAT_MMddyyyyHHmm));//默认结束日期
                } else {
                    switch (payProduct.getServiceLengthUnit()) {
                        case HOUR:
                            st.setServiceEndTime(DateUtil.addHours(DateUtil.strToDate(serviceTime, FORMAT_MMddyyyyHHmm), payProduct.getServiceLength()));
                            break;
                        case DAY:
                            st.setServiceEndTime(DateUtil.addDays(DateUtil.strToDate(serviceTime, FORMAT_MMddyyyyHHmm), payProduct.getServiceLength()));
                            break;
                        case MONTH:
                            st.setServiceEndTime(DateUtil.addMonths(DateUtil.strToDate(serviceTime, FORMAT_MMddyyyyHHmm), payProduct.getServiceLength()));
                            break;
                        default:
                            break;
                    }
                ...
                if (st.getServiceEndTime() == null) {
                    throw new HiveelCheckoutException(HiveelPayErrorCode.ILLEGAL_PARAMETERS, "Service end-time must not null and format like 'MMddyyyyHHmm'.");
                }
            }
            serviceTimes.add(st);
        }
        bizOrder.setServiceTimes(serviceTimes);
        bizOrder.setMchId(payRequestVo.getMchId());
        bizOrder.setChannelId(payRequestVo.getChannelId());
        bizOrder.setCurrency(payRequestVo.getCurrency());
        bizOrder.setRemark(Strings.emptyToNull(payRequestVo.getRemark()));
        bizOrder.setInvalidBizOrderNo(payRequestVo.getInvalidBizOrderNo());
        bizOrder.setDocId(payRequestVo.getDocId());

        /**
         * 暂时硬编码
         * 佣金调整改这里
         */
        if (payProduct.getProductType().equals(PayProductTypeEnum.TRADE_IN)) {
            bizOrder.setCommissionAmount(3000L);// 30  trade_in刀的佣金
        }

        if (payProduct.getProductType().equals(PayProductTypeEnum.ADVANCING) ||
                payProduct.getProductType().equals(PayProductTypeEnum.SEARCH_RESULT) ||
                payProduct.getProductType().equals(PayProductTypeEnum.CAR_OF_DAY) ||
                payProduct.getProductType().equals(PayProductTypeEnum.HIGHLIGHTING)
        ) {
            BizCar bc = new BizCar();
            bc.setBizOrderNo(bizOrderNo);
            bc.setCarId(bizOrder.getDocId());
            bc.setZipcode(payRequestVo.getDocZipcode());
            bc.setSyncStatus(SyncStatus.SAVED);
            bc.setNotifyCount(new Byte("0"));

            bizOrder.setBizCar(bc);


            int intervalDays = 0;
            for (ServiceTime st : serviceTimes) {
                intervalDays += DateUtil.getIntervalDays(st.getServiceStartTime(), st.getServiceEndTime());
            }
            Long totalAmount = intervalDays * payProduct.getAmount();

            bizOrder.setPayAmount(totalAmount);// 数量*单价
            bizOrder.setRemark(Strings.nullToEmpty(bizOrder.getRemark()) + ";" + intervalDays + "*" + payProduct.getAmount());
        } else {
            bizOrder.setPayAmount(payProduct.getAmount());
        }
        return bizOrder;
    }
     ...
     */
    @Override
    public BizOrder updatePaySuccess(String bizOrderNo) {
        BizOrder bizOrder = bizOrderMapper.selectByBizOrderNo(bizOrderNo);
        if (bizOrder == null || bizOrder.getOrderStatus().getVal() >= 6) {
            return null;
        }
        fillBizOrderServiceTime(bizOrder);
        _log.info("BizOrder pay success, bizOrderNo:{}", bizOrderNo);
        int i = bizOrderMapper.updatePaySuccess(bizOrderNo, BizOrderStatus.SAVED, BizOrderStatus.PAY_SUCCESSED);
        if (i != 1) {
            return bizOrder;
        }
        try {
            String productType = bizOrder.getProductType();
            if (isSpecialBusiness(productType, PayProductTypeEnum.OIL_CHANGE, PayProductTypeEnum.PRE_INSPECTION, PayProductTypeEnum.TRADE_IN)) {
                appointmentDocMapper.updateStatus(bizOrder.getDocId(), AppointmentStatus.SAVED, AppointmentStatus.VALID);
                _log.info("BizOrder pay success, update appointment:{}, status to valid.", bizOrder.getDocId());


            }
            if (bizOrder.getProductType().equalsIgnoreCase(PayProductTypeEnum.ADVANCING.name())) {// advancing 最新一次购买的有效

                Set<BizOrderStatus> orderStatusSet = Sets.newHashSet();
                orderStatusSet.add(BizOrderStatus.SERVICE_ING);

                List<BizOrder> bizOrderList = bizOrderMapper.searchBizOrdersByCustomerId(bizOrder.getCustomerId(), orderStatusSet, PayProductTypeEnum.ADVANCING.name());
                if (bizOrderList != null && !bizOrderList.isEmpty()) {
                    bizOrderList.forEach(item -> {
                        if (!item.getId().equals(bizOrder.getId())) {
                            bizOrderMapper.updateBizOrdersStatus(Collections.singletonList(item.getBizOrderNo()), BizOrderStatus.SERVICE_ING, BizOrderStatus.SERVICE_END);
                            List<CustomerValidServices> customerValidServices = customerValidServicesMapper.findByBizOrderNo(item.getBizOrderNo());
                            for (CustomerValidServices cs : customerValidServices) {
                                customerValidServicesMapper.updateStatusByServiceId(cs.getServiceId(), cs.getServiceStatus(), CustomerServiceStatusEnum.USED);
                            }
                        }
                    });
                }
            }
//            else if (bizOrder.getProductType().equalsIgnoreCase(PayProductTypeEnum.ADVANCING.name()) ||
//                    bizOrder.getProductType().equalsIgnoreCase(PayProductTypeEnum.CAR_OF_DAY.name()) ||
//                    bizOrder.getProductType().equalsIgnoreCase(PayProductTypeEnum.SEARCH_RESULT.name()) ||
//                    bizOrder.getProductType().equalsIgnoreCase(PayProductTypeEnum.HIGHLIGHTING.name())) {
//
//                BizCar bc = new BizCar();
//                bc.setBizOrderNo(bizOrderNo);
//                bc.setCarId(bizOrder.getDocId());
//                bc.setSyncStatus(SyncStatus.SAVED);
//                bc.setNotifyCount(new Byte("0"));
//                bizCarMapper.save(bc);
//
//                _log.info("BizOrder pay successed, notice to search-engine!,bizOrderNo:{},carId:{}", bizOrderNo, bizOrder.getDocId());
//            }

            notifyUser(bizOrder);
            addServiceAndUpdateBizOrder(bizOrder);
        } catch (Exception e) {
            _log.error("", e);
        }
        return bizOrder;
    ...
    private void notifyUser(BizOrder bizOrder) {
        final String productType = bizOrder.getProductType();
        if (productType.equalsIgnoreCase(PayProductTypeEnum.OIL_CHANGE.name())) {
            publisher.publishEvent(new NotificationEvent(NotificationEvent.ActionPoint.APPOINTMENT_BOOKED_OIL_CHANGE, NotificationEvent.NoticeChannel.EMAIL, bizOrder.getDocId()));
        } else if (productType.equalsIgnoreCase(PayProductTypeEnum.PRE_INSPECTION.name())) {
            publisher.publishEvent(new NotificationEvent(NotificationEvent.ActionPoint.APPOINTMENT_BOOKED_PRE_INSPECTION, NotificationEvent.NoticeChannel.EMAIL, bizOrder.getDocId()));
        } else if (productType.equalsIgnoreCase(PayProductTypeEnum.TRADE_IN.name())) {
            publisher.publishEvent(new NotificationEvent(NotificationEvent.ActionPoint.APPOINTMENT_BOOKED_TRADE_IN, NotificationEvent.NoticeChannel.EMAIL, bizOrder.getDocId()));
        }
    }
    ...
    public List<BizOrder> searchBizOrders(BizOrderSearchRequest searchRequest, HiveelPage hiveelPage) {
        ...
        checkNotNull(searchRequest.getMchId());
        checkNotNull(hiveelPage);
    ...
    @Override
    public void syncToSearchEngine(Set<String> bizOrders) {
        List<BizOrder> list = bizOrderMapper.findBizOrders(bizOrders);
        if (list == null || list.isEmpty()) {
            return;
        }
        Set<String> carIdSet = list.stream().filter(i -> i != null && !Strings.isNullOrEmpty(i.getDocId())).map(BizOrder::getDocId).collect(Collectors.toSet());
        if (carIdSet.isEmpty()) {
            return;
        }
        carIdSet.forEach(this::accept);
    }
...
//        statusEnums.add(CustomerServiceStatusEnum.IN_SERVICE);
    ...
    private void accept(String i) {
        try {
            notifySearchEngineComponent.notifySearchEngine(i);
        } catch (Exception e) {
            _log.error("", e);
        }
    }
