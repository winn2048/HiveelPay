...
public class PayOrderServiceImpl extends BaseService implements IPayOrderService {
    ...
    @Autowired
    private IPayChannel4HiveelService payChannel4HiveelService;
    ...
    public String doWxPayReq(String tradeType, JSONObject payOrder, String resKey) {
        ...
        String s = RpcUtil.mkRet(result);
        if (s == null) {
            return HiveelPayUtil.makeRetData(HiveelPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_FAIL, "0111", "调用微信支付失败"), resKey);
        }
        Map<String, Object> map = HiveelPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_SUCCESS, null);
        map.putAll((Map) result.get("bizResult"));
        return HiveelPayUtil.makeRetData(map, resKey);
    }
    ...
    public String doAliPayReq(String channelId, JSONObject payOrder, String resKey) {
        ...
        String s = RpcUtil.mkRet(result);
        if (s == null) {
            return HiveelPayUtil.makeRetData(HiveelPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_FAIL, "0111", "调用支付宝支付失败"), resKey);
        }
        Map<String, Object> map = HiveelPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_SUCCESS, null);
        map.putAll((Map) result.get("bizResult"));
        return HiveelPayUtil.makeRetData(map, resKey);
     ...
     */
    @Override
    public String doHiveelPayReq(String channelId, JSONObject payOrder, String resKey) {
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("payOrder", payOrder);
        String jsonParam = RpcUtil.createBaseParam(paramMap);
        Map<String, Object> result;
        switch (channelId) {
            case PayConstant.PAY_CHANNEL_H_CHARGE_SUBSCRIPTION:
                result = payChannel4HiveelService.doHiveelPayChargeAndSubscriptionReq(jsonParam);
                break;
            case PayConstant.PAY_CHANNEL_H_SUBSCRIPTION:
                result = payChannel4HiveelService.doHiveelPaySubscriptionReq(jsonParam);
                break;
            case PayConstant.PAY_CHANNEL_H_ONCE:
                result = payChannel4HiveelService.doHiveelPayOnceReq(jsonParam);
                break;
            default:
                result = null;
                break;
        ...
        String s = RpcUtil.mkRet(result);
        if (s == null) {
            return HiveelPayUtil.makeRetData(HiveelPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_FAIL, "0111", "调用H_SUBSCRIPTION  支付失败"), resKey);
        }
        Map<String, Object> map = HiveelPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS, "", PayConstant.RETURN_VALUE_SUCCESS, null);
        map.putAll((Map) result.get("bizResult"));
        return HiveelPayUtil.makeRetData(map, resKey);
